...
### 2 - Local JSON structure

**Local JSON** files are generated during the test configuration process and are saved in both external (`/storage/emulated/0/Android/data/com.example.epilepsytestapp/files/EpilepsyTests/Configurations/configuration_yyyy-MM-dd_HH:mm.json`) and internal (`/data/user/0/com.example.epilepsytestapp/files/localtestconfiguration.json`) storage locations of the app.  
These files contain the list of tests selected for a specific patient and are used during the testing phase to ensure the correct tests are administered.
