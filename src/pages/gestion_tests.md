...
More information about how tests are used can be found in the [next part](testing.md).
...
Tests' configuration must be done after **signing up**, and can be accessed via the [Settings page](pages_application.md-dans-la-section-settingspage.kt) by clicking on the button `"Modifier la configuration"`. This will display an alert dialog warning users that modifying the configuration requires supervision by a qualified medical professional.
...
This screen lists all the previous configurations (local JSON files) that have been saved in the form of clickable boxes. It can be accessed from the Configuration screen [ConfigScreen.kt](#**_3._recap_and_save_–_`recapconfigscreen.kt`**).    
