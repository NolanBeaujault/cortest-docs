...
- a custom **top bar** with the app logo, the `"Home"` title, and a profile icon that links to the <Link to="/wait">profile page</Link>,
...
- <Link to="/wait">`DemoScreen.kt`</Link>,
- <Link to="/wait">`FilesPage.kt`</Link>,
- <Link to="/wait">`SettingsPage.kt`</Link>.
